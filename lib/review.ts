...
export function getReviewIssues(data:RamsData):ReviewIssue[]{
  ...
  if(hasWork(data,'Trenching')&&isMissing(data.trenchingMethod))add({id:'method-trenching',severity:'warning',blocking:false,category:'method',step:7,title:'Trenching method missing',description:'Trenching is selected. Add excavation, service avoidance, trench protection and reinstatement controls.',fieldId:'trenching-method',actionLabel:'Review Method'});
  if(hasWork(data,'EV charger')&&isMissing(data.evChargerInstallationMethod))add({id:'method-ev',severity:'warning',blocking:false,category:'method',step:7,title:'EV charger method missing',description:'EV charger work is selected. Add the charger installation, commissioning and test-record method.',fieldId:'ev-charger-installation-method',actionLabel:'Review Method'});
