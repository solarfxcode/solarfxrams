...
type MethodRule = {id: string; field: keyof RamsData; fieldId: string; title: string; description: string; workTypes?: string[]; always?: boolean};
type RiskRequirement = {workTypes: string[]; id: string; title: string; terms: string[]};
...
const methodRules: MethodRule[] = [
  {id: 'method-general', field: 'methodStatement', fieldId: 'field-method-statement', title: 'General method missing', description: 'Add the editable safe system of work.', always: true},
  {id: 'method-delivery-storage', field: 'deliveryStorageMethod', fieldId: 'delivery-storage-method', title: 'Delivery and storage method missing', description: 'Add the delivery, storage, inspection and handling method.', always: true},
  {id: 'method-access', field: 'accessMethod', fieldId: 'access-method', title: 'Access method missing', description: 'Access equipment or roof work is selected. Add the access method.', workTypes: ['Roof work', 'Scaffold', 'Mobile tower', 'MEWP', 'Ladder access']},
  {id: 'method-roof', field: 'roofWorkMethod', fieldId: 'roof-work-method', title: 'Roof work method missing', description: 'Solar PV or roof work is selected. Add the roof work method.', workTypes: ['Solar PV', 'Roof work']},
  {id: 'method-dc', field: 'dcInstallationMethod', fieldId: 'dc-installation-method', title: 'DC installation method missing', description: 'Solar PV is selected. Add the DC installation method.', workTypes: ['Solar PV']},
  {id: 'method-inverter', field: 'inverterInstallationMethod', fieldId: 'inverter-installation-method', title: 'Inverter installation method missing', description: 'Solar PV is selected. Add the inverter installation method.', workTypes: ['Solar PV']},
  {id: 'method-battery', field: 'batteryInstallationMethod', fieldId: 'battery-installation-method', title: 'Battery installation method missing', description: 'Battery storage is selected. Add the battery handling, mounting, clearances, ventilation and emergency method.', workTypes: ['Battery storage']},
  {id: 'method-ev', field: 'evChargerInstallationMethod', fieldId: 'ev-charger-installation-method', title: 'EV charger installation method missing', description: 'EV charger work is selected. Add the charger location, route, isolation, commissioning and handover method.', workTypes: ['EV charger']},
  {id: 'method-trenching', field: 'trenchingMethod', fieldId: 'trenching-method', title: 'Trenching method missing', description: 'Trenching is selected. Add the excavation, service avoidance, trench protection and reinstatement method.', workTypes: ['Trenching']},
  {id: 'method-consumer-unit', field: 'consumerUnitMethod', fieldId: 'consumer-unit-method', title: 'Consumer unit method missing', description: 'Consumer unit work is selected. Add the safe isolation, circuit identification, installation and labelling method.', workTypes: ['Consumer unit work']},
  {id: 'method-loft', field: 'loftWorkMethod', fieldId: 'loft-work-method', title: 'Loft work method missing', description: 'Loft work is selected. Add the loft access, lighting, fragile ceiling, dust and route method.', workTypes: ['Loft work']},
  {id: 'method-external-cable', field: 'externalCableRouteMethod', fieldId: 'external-cable-route-method', title: 'External cable route method missing', description: 'External cable route or EV charger work is selected. Add route inspection, protection, sealing and trip controls.', workTypes: ['External cable route', 'EV charger']},
  {id: 'method-eps', field: 'epsBackupMethod', fieldId: 'eps-backup-method', title: 'EPS / backup method missing', description: 'EPS or backup work is selected. Add circuit identification, separation, labelling and functional-check method.', workTypes: ['EPS / backup']},
  {id: 'method-ac', field: 'acConnectionMethod', fieldId: 'ac-connection-method', title: 'AC connection method missing', description: 'Electrical connection work is selected. Add the AC isolation, containment, termination and restoration method.', workTypes: ['Solar PV', 'Battery storage', 'EV charger', 'EPS / backup', 'Consumer unit work']},
  {id: 'method-commissioning', field: 'commissioningHandoverMethod', fieldId: 'commissioning-handover-method', title: 'Commissioning and handover method missing', description: 'Add manufacturer commissioning, functional checks, labels, monitoring setup and handover details.', always: true}
];

const riskRequirements: RiskRequirement[] = [
  {workTypes: ['Solar PV'], id: 'risk-work-solar-pv', title: 'Solar PV risk rows missing', terms: ['solar', 'pv', 'module', 'dc', 'inverter', 'roof', 'electrical']},
  {workTypes: ['Roof work'], id: 'risk-work-roof', title: 'Roof work risk rows missing', terms: ['roof', 'height', 'fall', 'scaffold', 'ladder']},
  {workTypes: ['Battery storage'], id: 'risk-work-battery', title: 'Battery risk rows missing', terms: ['battery', 'thermal', 'clearance', 'ventilation']},
  {workTypes: ['EV charger'], id: 'risk-work-ev', title: 'EV charger risk rows missing', terms: ['ev', 'charger', 'vehicle', 'external cable']},
  {workTypes: ['Trenching'], id: 'risk-work-trenching', title: 'Trenching risk rows missing', terms: ['trench', 'excavat', 'underground', 'service strike', 'cat and genny']},
  {workTypes: ['Consumer unit work'], id: 'risk-work-consumer-unit', title: 'Consumer unit risk rows missing', terms: ['consumer unit', 'safe isolation', 'lock-off', 'electrical']},
  {workTypes: ['Loft work'], id: 'risk-work-loft', title: 'Loft work risk rows missing', terms: ['loft', 'fragile ceiling', 'restricted space']},
  {workTypes: ['External cable route'], id: 'risk-work-external-cable', title: 'External cable route risk rows missing', terms: ['external cable', 'cable route', 'penetration', 'trip']},
  {workTypes: ['EPS / backup'], id: 'risk-work-eps', title: 'EPS / backup risk rows missing', terms: ['eps', 'backup', 'critical-load', 'separation']}
];

function issue(input: IssueInput): ReviewIssue {
  return {severity: input.severity ?? (input.blocking === false ? 'warning' : 'error'), blocking: input.blocking ?? true, stepLabel: REVIEW_STEP_LABELS[input.step] ?? 'Review', ...input};
}

function methodApplies(rule: MethodRule, data: RamsData) {
  if (rule.always) return true;
  return (rule.workTypes || []).some(workType => hasWork(data, workType));
}

function riskMatchesTerms(risk: {hazard: string; activity?: string; category?: string; source?: string}, terms: string[]) {
  const haystack = lower([risk.hazard, risk.activity, risk.category, risk.source].filter(Boolean).join(' '));
...
export function getReviewIssues(data: RamsData): ReviewIssue[] {
  ...
  if (isMissing(data.siteSupervisor)) add({id: 'job-site-supervisor', category: 'job', step: 0, title: 'Site supervisor missing', description: 'Name the site supervisor responsible for briefing and monitoring controls.', fieldId: 'field-site-supervisor', actionLabel: 'Go to Job'});
  if (systemTypes.some(type => ['Solar PV', 'Battery storage', 'EV charger', 'Consumer unit work', 'EPS / backup'].includes(type)) && isMissing(data.qualifiedElectrician)) add({id: 'job-qualified-electrician', category: 'job', step: 0, title: 'Qualified electrician missing', description: 'Name the qualified electrician for electrical installation, isolation and commissioning.', fieldId: 'field-qualified-electrician', actionLabel: 'Go to Job'});
  ...
  if (!Array.isArray(data.personsAtRisk) || data.personsAtRisk.length === 0) add({id: 'system-persons-at-risk', category: 'system', step: 2, title: 'Persons at risk missing', description: 'Select the persons who may be affected by the work.', fieldId: 'persons-at-risk-list', actionLabel: 'Go to System'});
  if (!Array.isArray(data.equipment) || data.equipment.length === 0) add({id: 'system-equipment', category: 'system', step: 2, title: 'Plant, tools and equipment missing', description: 'Select the plant, tools and equipment expected for this job.', fieldId: 'equipment-list', actionLabel: 'Go to System'});
  ...
  if ((hasWork(data, 'Roof work') || hasWork(data, 'Scaffold') || hasWork(data, 'Ladder access')) && isMissing(data.accessNotes)) add({id: 'site-access-method', category: 'site', step: 3, title: 'Access arrangements missing', description: 'Record how the roof or work area will be accessed safely.', fieldId: 'field-access-notes', actionLabel: 'Go to Site'});
  ...
  photos.forEach((photo, index) => {
    ...
    if (!photo.includeInPdf) return;
    if (isMissing(photo.category)) add({id: 'photo-' + photo.id + '-category', severity: 'warning', blocking: false, category: 'photos', step: 4, title: 'Selected photo category missing', description: 'Photo ' + (index + 1) + ' is selected for the PDF and needs a category.', fieldId: photoTarget, photoId: photo.id, actionLabel: 'Review Photo ' + (index + 1)});
    if (isMissing(photo.caption)) add({id: 'photo-' + photo.id + '-caption', severity: 'warning', blocking: false, category: 'photos', step: 4, title: 'Selected photo caption missing', description: 'Photo ' + (index + 1) + ' is selected for the PDF and needs a caption for context.', fieldId: photoTarget, photoId: photo.id, actionLabel: 'Review Photo ' + (index + 1)});
  });
  ...
  if (risks.length === 0) add({id: 'risk-none', category: 'risk', step: 6, title: 'No risk assessment rows', description: 'Add risk rows or use recommended risks for the selected work activities.', fieldId: 'risk-list', actionLabel: 'Review Risk'});
  riskRequirements.forEach(requirement => {
    const applies = requirement.workTypes.some(workType => hasWork(data, workType));
    if (!applies || risks.length === 0) return;
    if (!risks.some(risk => riskMatchesTerms(risk, requirement.terms))) add({id: requirement.id, category: 'risk', step: 6, title: requirement.title, description: 'Add or keep at least one risk row relevant to the selected ' + requirement.workTypes.join(' / ') + ' activity.', fieldId: 'risk-list', actionLabel: 'Review Risk'});
  });
  risks.forEach((risk, index) => {
    ...
    if (isMissing(risk.persons)) add({id: 'risk-' + risk.id + '-persons', category: 'risk', step: 6, title: 'Persons at risk missing', description: 'Complete persons at risk for ' + rowName + '.', fieldId: rowTarget, riskId: risk.id, actionLabel: 'Review Risk'});
  ...
  hazards.filter(hazard => hazard.status === 'accepted').forEach((hazard, index) => {
    if (!risks.some(risk => risk.source === hazard.id)) add({id: 'risk-ai-' + hazard.id + '-missing', category: 'risk', step: 6, title: 'Accepted AI hazard missing from risk assessment', description: (hazard.title || 'Accepted AI hazard ' + (index + 1)) + ' is accepted but not represented in the risk rows.', fieldId: 'risk-list', entityId: hazard.id, actionLabel: 'Review Risk'});
  });
  ...
  methodRules.forEach(rule => {
    if (!methodApplies(rule, data)) return;
    if (isMissing(data[rule.field])) add({id: rule.id, category: 'method', step: 7, title: rule.title, description: rule.description, fieldId: rule.fieldId, actionLabel: 'Review Method'});
  });
  if (!Array.isArray(data.ppe) || data.ppe.length === 0) add({id: 'method-ppe', category: 'method', step: 7, title: 'PPE selection missing', description: 'Select the PPE required for the job.', fieldId: 'ppe-matrix-list', actionLabel: 'Review Method'});
  ...
  if (isMissing(data.firstAidKitLocation)) add({id: 'emergency-first-aid-kit', category: 'emergency', step: 8, title: 'First aid kit location missing', description: 'Record where the first aid kit will be available.', fieldId: 'field-first-aid-kit-location', actionLabel: 'Go to Emergency'});
  ...
  if (isMissing(data.fireExtinguisherLocation)) add({id: 'emergency-fire-extinguisher', category: 'emergency', step: 8, title: 'Fire extinguisher location missing', description: 'Record the fire extinguisher location or state the site arrangement.', fieldId: 'field-fire-extinguisher-location', actionLabel: 'Go to Emergency'});
  if (systemTypes.some(type => ['Solar PV', 'Battery storage', 'EV charger', 'Consumer unit work', 'EPS / backup'].includes(type)) && isMissing(data.siteIsolationPoint)) add({id: 'emergency-site-isolation', category: 'emergency', step: 8, title: 'Site isolation point missing', description: 'Record the main isolation point or electrical isolation arrangement.', fieldId: 'field-site-isolation-point', actionLabel: 'Go to Emergency'});
  ...
  if (hasWork(data, 'EV charger') && isMissing(data.evShutdownInfo)) add({id: 'emergency-ev-shutdown', category: 'emergency', step: 8, title: 'EV shutdown information missing', description: 'EV charger work is selected. Add the EV charger shutdown or isolation information.', fieldId: 'field-ev-shutdown', actionLabel: 'Go to Emergency'});
...
export function getNextReviewIssue(issues: ReviewIssue[]): ReviewIssue | undefined {
  return issues.find(item => item.blocking) || issues.find(item => item.severity === 'warning') || issues[0];
}

export function hasBlockingReviewIssues(issues: ReviewIssue[]): boolean {
  return issues.some(item => item.blocking);
}

export function getReviewStepSummaries(issues: ReviewIssue[]) {
  return REVIEW_STEP_LABELS.map((label, step) => {
    const stepIssues = issues.filter(item => item.step === step);
    const errors = stepIssues.filter(item => item.blocking).length;
    const warnings = stepIssues.filter(item => !item.blocking).length;
    return {step, label, errors, warnings, total: stepIssues.length, state: errors ? 'error' : warnings ? 'warning' : 'complete'};
  });
}

export function isPdfReadyFromIssues(issues: ReviewIssue[]): boolean {
  return !hasBlockingReviewIssues(issues);
}

export const REVIEW_RENDERED_METHOD_TARGETS = methodRules.map(rule => rule.fieldId);
export const REVIEW_RISK_WORK_IDS = riskRequirements.map(rule => slug(rule.id));
