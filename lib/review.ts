...
export function getReviewIssues(data:RamsData):ReviewIssue[]{
  ...
  if(hasWork(data,'Battery storage')&&isMissing(data.batteryLocation))add({id:'system-battery-location',category:'system',step:2,title:'Battery location missing',description:'Battery storage is selected, so add the proposed battery location.',fieldId:'field-battery-location',actionLabel:'Go to System'});
  if(hasWork(data,'EV charger')&&isMissing(data.evChargerLocation))add({id:'system-ev-location',category:'system',step:2,title:'EV charger location missing',description:'EV charger work is selected, so add the proposed charger location.',fieldId:'field-ev-location',actionLabel:'Go to System'});
  ...
  if(hasWork(data,'Battery storage')&&!includesAny(data.methodStatement,['battery','storage']))add({id:'method-battery',severity:'warning',blocking:false,category:'method',step:7,title:'Battery installation method missing',description:'Battery storage is selected. Add battery installation and isolation details to the method statement.',fieldId:'field-method-statement',actionLabel:'Review Method'});
